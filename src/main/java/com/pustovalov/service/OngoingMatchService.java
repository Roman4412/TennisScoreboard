...
public class OngoingMatchService {
    ...
    public UUID create(CreateMatchDto createMatchDto) {
        ...
        Player playerOne = playerPersistenceService.findBy(playerOneName).orElseGet(
                () -> playerPersistenceService.persist(new Player(playerOneName)));

        Player playerTwo = playerPersistenceService.findBy(playerTwoName).orElseGet(
                () -> playerPersistenceService.persist(new Player(playerTwoName)));
